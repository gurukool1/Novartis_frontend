...
import { getSubmittedCases,triggerEvaluation } from '../../Redux/Actions/CaseAction';
//import { triggerEvaluation } from "../../Redux/Actions/Actions";
...
export const SubmittedForm = () => {
   ...
   //const { caseId, formId } = useSelector(state => state.forms);
    ...
    // const { caseId, formId } = useSelector(state => state.case); 
...
const handleCompareAnswerSheet = async () => {
        if (!caseId || !formId) {
        ...
        try {
            // triggerEvaluation returns the full API response:
            // { status: true, data: { formData: {...}, result: {...} } }
            const result = await dispatch(triggerEvaluation(caseId, formId));

            if (result?.status === true) {
                navigate("/admin/answer-sheet/compare", {
                    state: {
                        evaluationData: result,          // full API response
                        backPath: location.pathname + location.search,
                        caseId,                          // needed by AnswerSheetComparison to call loadForm
                        formId,
                        userCaseId,
                        selectedForms,                   // needed to know which forms to render
                    },
                });
            } else {
                alert(result?.error || "Failed to fetch evaluation data. Please try again.");
            }
        } catch (err) {
            console.error("Evaluation error:", err);
            alert("An error occurred while fetching evaluation data.");
        } finally {
            ...
            <MainWrapper>
                <div className="d-flex justify-content-end mt-3 mb-1">
                    <button
                        ...
                        onClick={handleCompareAnswerSheet}
...
// I want to show the answer sheet of the user and compare it with the answer sheet filled by the doctor. I have created a new action in case action file named triggerEvaluation which will trigger the evaluation api and get the response. I want to show that response in a new page where i can compare the answersheet of user and doctor side by side. craete new component for that named AnswerSheetComparison.jsx. I want to navigate to that page on click of a button - Compare AnswerSheet in SubmittedForm.jsx and pass the response of evaluation api to that page. Please help me with the code for that.
// new endpoint is lieked this - "evaluation/trigger" with payload { caseId, formId } and it will return the answer sheet of user and doctor in response. I want to show that response in a new page named AnswerSheetComparison.jsx where i can compare the answersheet of user and doctor side by side. I want to navigate to that page on click of a button - Compare AnswerSheet in SubmittedForm.jsx and pass the response of evaluation api to that page. Please help me with the code for that.
// if the data is mistmatched the show the mismatched data in red and if it is matched show in green. also show the percentage of matching at the top of the page. also add a back button to go back to the submitted form page.
...
//         (item) => item.id === userCaseId && item.formId === formId
//     ) || {};
...
//                 <div className={`row g-3 mt-1 ${isReadOnly ? "user-submit-form" : ""}`}>
//                     {(expandedSection === null || expandedSection === "caseInfo") && (
//                         <div
...
//                             <div className="outer-wrap">
//                                 <div
//                                     className="theme-card block-height"
...
//                                         )}
//                                         {shouldShowForm8 && (
//                                             <Form8 visit="initial" readOnly={isReadOnly} />
//                                         )}
//                                         {shouldShowForm(3) && (
//                                             <Form7 visit="initial" readOnly={isReadOnly} FORM_COUNT={totalFormCount} />
//                                         )}
...
//                                         )}
//                                         {shouldShowForm8 && (
//                                             <Form8 visit="followUp" readOnly={isReadOnly} />
//                                         )}
//                                         {shouldShowForm(3) && (
//                                             <Form7 visit="followUp" readOnly={isReadOnly} FORM_COUNT={totalFormCount} />
//                                         )}
